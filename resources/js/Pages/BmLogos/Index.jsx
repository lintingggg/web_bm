...
export default function BmLogosIndex({ logos, stats }) {
    ...
    const submit = (event) => {
        ...
        if (editingItem) {
            ...
            form.post(route('bm-logos.update', editingItem.id), {
                preserveScroll: true,
                onSuccess: () => {
                    resetForm();
                },
        ...
        form.post(route('bm-logos.store'), {
            preserveScroll: true,
            onSuccess: () => {
                resetForm();
            },
    ...
    const deleteItem = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data logo BM ini?')) {
            router.delete(route('bm-logos.destroy', id), {
                preserveScroll: true,
            });
        }
    };
    ...
    const toggleActive = (item) => {
        router.put(route('bm-logos.update', item.id), {
            _method: 'put',
            title: item.title,
            description: item.description,
    ...
    return (
        <AuthenticatedLayout
            header={<h2 className="text-xl font-semibold leading-tight text-neutral-800 dark:text-neutral-200">Kelola Logo BM</h2>}
        >
            <Head title="Logo BM - Admin" />
                            ...
                            >
                                <div className="border-b border-neutral-200 px-6 py-4 dark:border-neutral-800">
                                    <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100">
                                        {editingItem ? 'Edit Logo BM' : 'Tambah Baru'}
                                    </h3>
                                </div>
                                ...
                                <div className="flex flex-col gap-4 p-6">
                                    <div className="space-y-1">
                                        <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                            Judul Logo <span className="text-neutral-400 text-xs">(Opsional)</span>
                                        </label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                            Deskripsi & Filosofi <span className="text-neutral-400 text-xs">(Opsional)</span>
                                        </label>
                                        <textarea
                                            value={form.data.description}
                                            onChange={(e) => form.setData('description', e.target.value)}
                                            className="w-full rounded-lg border-neutral-300 bg-white text-sm focus:border-accent-orange focus:ring-accent-orange dark:border-neutral-700 dark:bg-surface-muted dark:text-white"
                                            placeholder="Makna atau filosofi logo"
                                            rows="4"
                                        ></textarea>
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                            Gambar Logo <span className="text-rose-500">*</span> <span className="text-xs text-neutral-400">(Max 2MB)</span>
                                        </label>
                                        <div 
                                ...
                                <div className="flex items-center justify-end gap-3 border-t border-neutral-200 px-6 py-4 dark:border-neutral-800">
                                    ...
                                    >
                                        {form.processing ? (
                                            ...
                                            <>
                                                <Plus size={16} />
                                                {editingItem ? 'Simpan Perubahan' : 'Tambah Logo'}
                                            </>
                                        )}
                                    </button>
                                </div>
                                                ...
                                                logos.map((item) => (
                                                    <tr key={item.id} className="hover:bg-neutral-50 dark:hover:bg-surface-muted/50">
                                                        <td className="px-6 py-4">
                                                            {item.image_url ? (
                                                                <div className="h-16 w-16 bg-neutral-100 dark:bg-neutral-800 rounded-lg flex items-center justify-center p-2 border border-neutral-200 dark:border-neutral-700">
                                                                    <img 
                                                                        src={item.image_url} 
                                                                        alt={item.title || 'Logo BM'} 
                                                                        className="max-h-full max-w-full object-contain" 
                                                                    />
                                                                </div>
                                                            ) : (
                                                                <div className="h-16 w-16 bg-neutral-100 dark:bg-neutral-800 rounded-lg flex items-center justify-center text-xs text-neutral-400 border border-neutral-200 dark:border-neutral-700">
                                                                    No Image
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            ...
                                                            {item.description && (
                                                                <div className="text-xs text-neutral-500 mt-1 line-clamp-1 truncate max-w-[200px]" title={item.description}>
                                                                    {item.description}
                                                                </div>
                                                            )}
                                                        </td>
